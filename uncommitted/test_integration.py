    ...
    for system in "git", "hg", "svn":
        for state in "clean", "dirty", "ignore":
            ...
            if system != "svn":
                ...
                if system == "hg":
                    cc(
                        [system] + hg_identity + ["commit", "-m", "Add more maxim"],
                        cwd=d,
                    )
                else:
...
def clones(git_identity, tempdir, checkouts, cc):
    ...
    for behind in False, True:
        for ahead in False, True:
            ...
            cc(
                [system, "checkout", "-b", local_branch, "origin/master"],
            ...
            if ahead:
                file_to_edit = os.path.join(complex_clone_dir, filename)
                with open(file_to_edit, "ab") as f:
                    f.write(even_more_maxim)
                cc([system, "add", filename], cwd=complex_clone_dir)
                cc([system, "commit", "-m", "Even more maxim"], cwd=complex_clone_dir)
...
def test_unpushed(clones):
    ...
    # star):
    expected_output_regex = re.compile(
        dedent(
            """\
...
def test_follow_symlinks(checkouts):
    ...
    try:
        ...
        # `git-clean`:
        if sys.platform == "win32":
            call(["mklink", "/J", symlink, pointed_repo], shell=True)
        else:
...
def svn_locked(tempdir, cc):
    ...
    cc(["svn", "add", file_to_lock], cwd=locked)
    cc(["svn", "commit", file_to_lock, "-m", "add file to be locked"], cwd=locked)
...
def test_symlink_loop(checkouts):
    ...
    try:
        ...
        if sys.platform == "win32":
            call(["mklink", "/J", symlink, repo_with_symlink], shell=True)
        else:
